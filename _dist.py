...
def dist_uhd_pwr_by_site(df, out_dir, grid_size):
    SUBPLOT_HEIGHT = 600
    TOP_MARGIN = 70
    LEGEND_Y = 1.08
    LEGEND_FONT_SIZE = 13
...
def dist_kpis_by_site(df, out_dir, rb_min):
    SUBPLOT_HEIGHT = 600
...
def dist_kpis_by_site_rsrp_bin(df, out_dir, rb_min, rsrp_bin):
    ...
    for b_idx, b in enumerate(bins[:-1]):
        ...
        for band_name in band_list:
            ...
            for i, (metric, x_title, x_range) in enumerate(metrics, start=1):
                for route_name, color in route_colors.items():
                    ...
                    data = group[metric].dropna().values
                    total_count = len(data)
                    dist_bin_size = 15 if total_count < 1000 else 30
        ...
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, f"kpis_by_site_rsrp_bin_{rsrp_bin}dB")
        os.makedirs(out_path, exist_ok=True)
        out_path = os.path.join(out_path, f"RSRP_{rsrp_min}_to_{rsrp_max}.html")
        fig.write_html(out_path)
        print(f"✅ Saved: {out_path}")

def dist_kpis_by_band(df, out_dir, rb_min, rsrp_bin):
    SUBPLOT_HEIGHT = 600
    VERTICAL_SPACING = 0.035
    TOP_MARGIN = 70
    LEGEND_Y = 1.03
...
def dist_kpis_by_site_pdf_cdf(df, out_dir, rb_min, rsrp_bin):
    SUBPLOT_HEIGHT = 600
    VERTICAL_SPACING = 0.035
    TOP_MARGIN = 70
    LEGEND_Y = 1.03
...
def dist_kpis_by_band_pdf_cdf(df, out_dir, rb_min, rsrp_bin):
    SUBPLOT_HEIGHT = 600
    VERTICAL_SPACING = 0.035
    TOP_MARGIN = 70
    LEGEND_Y = 1.03
