...
def dist_kpis_pdf_rsrp_bins(df, out_dir, rb_min, sample_min, rsrp_bin):
    ...
    for b_idx, b in enumerate(bins[:-1]):
        ...
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, f"kpis_pdf_rsrp_bins")
        os.makedirs(out_path, exist_ok=True)
        out_path = os.path.join(out_path, f"RSRP_{rsrp_min}_to_{rsrp_max}.html")
        fig.write_html(out_path)
        print(f"✅ Saved: {out_path}")

def dist_kpis_pdf_by_band(df, out_dir, rb_min):
    SUBPLOT_HEIGHT = 600
    VERTICAL_SPACING = 0.035
    TOP_MARGIN = 70
    LEGEND_Y = 1.03
...
def dist_kpis_pdf_by_uhd(df, out_dir, rb_min, grid_size):
    ...
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"kpis_pdf_by_uhd.html")
    fig.write_html(out_path)
    print(f"✅ Saved: {out_path}")
...
def dist_kpis_cdf_by_band(df, out_dir, rb_min):
    ...
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"kpis_cdf_by_band.html")
    fig.write_html(out_path)
    print(f"✅ Saved: {out_path}")
def dist_kpis_cdf_by_site(df, out_dir, rb_min):
    SUBPLOT_HEIGHT = 600
    VERTICAL_SPACING = 0.035
    TOP_MARGIN = 70
    LEGEND_Y = 1.03
...
def dist_kpis_cdf_by_uhd(df, out_dir, rb_min, grid_size):
    ...
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"kpis_cdf_by_uhd.html")
    fig.write_html(out_path)
    print(f"✅ Saved: {out_path}")
