    ...
    for route_name in route_list:
        ...
        for i, (metric, y_title) in enumerate(metrics, start=1):
            for band in order:
                ...
                low = group[metric].quantile(0.01)
                valid = group[group[metric] >= low].copy()
...
def plot_kpi_group_by_uhd(df, out_dir, grid_size, rb_min, sample_min, band):
    ...
    for metric in metrics:
        ...
        if band == "n28":
            map_dir = '/'.join(out_dir.split("/")[0:2])
            map_url = f"https://joostone-ahn.github.io/nr-field-analysis/{map_dir}/map/n28_DL_Tput.html"
            title_text = (
